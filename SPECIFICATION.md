...
1. **資料來源優先順序**：
   ...
   - 當環境部 API 無法使用或傳回 0 筆資料時，則自動退回 **(3) 本地分區離線資料**（經由地理邊界動態載入的備份數據），實現多重保險與離線存取。
2. **台灣環境部 (MOENV) API**：
...
1. **分區資料庫**：
   - 全台公廁及本地超商/商場數據在建置時，透過 `scripts/compile_counties.js` 腳本按全台 22 縣市的地理邊界切割，分別儲存為 `data/<county_key>.json` 小檔案（單檔大小在 200KB 以下），大幅減輕行動網路與手機記憶體負擔。
2. **智慧邊界偵測與合併**：
