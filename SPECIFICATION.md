...
### A. 雙數據源與優先級載入 (Data Sources & Fallback Priority)
1. **資料來源優先順序**：
   - 系統預設首選為 **(1) OpenStreetMap**，提供全球最新的即時地標資料。
   - 當 OSM 載入失敗時，系統將主動且無縫地切換至 **(2) 環境部 Open Data**。
   - 當環境部 API 亦無法使用時，則自動退回 **(3) 本地離線資料**（備份示範數據），實現多重保險。
2. **台灣環境部 (MOENV) API**：
...
1. **首批語系支援**：
   - 支援台灣正體中文 (`zh-TW`)、英文 (`en`)、日文 (`ja`)、瑞典文 (`sv`) 四種語系。
2. **啟動自動偵測機制**：
   - App 啟動時，優先使用 `navigator.language` 偵測使用者手機或瀏覽器的系統語系。
   - 匹配規則：偵測到中文語系（`zh`）使用 `zh-TW`，日文使用 `ja`，瑞典文使用 `sv`，若系統語言非上述支援語系，則一律 fallback 顯示「英文 (`en`)」介面。
3. **使用者設定與記憶**：
